	...
	for code in range(len(codes)):
		...
		# ADDITIONAL FIELDS III: METALLICITY (IN MASS FRACTION, NOT IN ZSUN)
			...
			# We are creating ("Gas", "Metallicity") here, different from ("Gas", "metallicity") which is auto-generated by yt but doesn't work properly
 		  	...
 		  	num_neighbors = 64
 		  	fn = add_volume_weighted_smoothed_field(PartType_Gas_to_use, "Coordinates", MassType_to_use, "SmoothingLength", "Density", MetallicityType_to_use, pf.field_info, num_neighbors)
 		  	# Somehow a simple alias doesn't work (e.g. pf.field_info.alias(("gas", "metallicity"), fn[0]) ) probably because pf=GadgetDataset(), not load()
			# so I force to replace the existing ("gas", "metallicity") by adding a new field
